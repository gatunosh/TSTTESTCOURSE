...
export enum PasswordErrors {
    ...
    NOT_UPPER_CASE = 'Upper case letter required',
    NOT_LOWER_CASE = 'Lower case letter required',
    NOT_NUMBER = 'At least one number required'
}
...
export class PasswordChecker {
    ...
    private checkForNumber(password: string, reasons: PasswordErrors[]) {
        const hasNumber = /\d/;
        if (!hasNumber.test(password)) reasons.push(PasswordErrors.NOT_NUMBER);
    }
    ...
    private checkForUpperCase(password: string, reasons: PasswordErrors[]) {
        if (password === password.toLowerCase()) reasons.push(PasswordErrors.NOT_UPPER_CASE);
    }

    private checkForLowerCase(password: string, reasons: PasswordErrors[]) {
        if (password === password.toUpperCase()) reasons.push(PasswordErrors.NOT_LOWER_CASE);
    }
